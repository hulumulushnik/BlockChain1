...
namespace BlockChainP411NEW.Services
{
    public class TransactionService
    {
        ...
        public (bool IsValid, string ErrorMessage) ValidateTransaction(Transaction transaction)
        {
            ...
            // Сувора валідація крипто-адрес
            if (!IsValidCryptoAddress(transaction.From)) return (false, "Invalid Sender (From) address format. Must be 42 chars, start with 0x, and be alphanumeric.");
            if (!IsValidCryptoAddress(transaction.To)) return (false, "Invalid Recipient (To) address format. Must be 42 chars, start with 0x, and be alphanumeric.");
