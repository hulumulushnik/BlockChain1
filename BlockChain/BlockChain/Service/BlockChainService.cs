    ...
    public class BlockChainService
    {
        ...
        public BlockChainService()
        {
            ...
            if (loadedChain != null && loadedChain.Count > 0)
            {
                ...
                Console.WriteLine($"[Сховище] Завантажено ланцюг із {Chain.Count} блоків.");
            }
            else
            {
                _transactionService = new TransactionService(_walletService) { BlockChain = this };
        ...
        public void MineBlock(string minerAddress)
        {
            ...
            if (TotalMinted + totalReward > MaxSupply)
                totalReward = MaxSupply - TotalMinted;
        ...
        public bool IsValid()
        {
            for (int i = 1; i < Chain.Count; i++)
            {
                ...
                foreach (var tx in current.Transactions)
                {
                    ...
                    bool isValidSignature = _walletService.VerifySignature(tx.From, tx.GetDataToSign(), tx.Signature);
